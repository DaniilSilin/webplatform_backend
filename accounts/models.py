...
class UniversalVerification(models.Model):
    ...
    creation_id = models.BigIntegerField(
        unique=True, null=True, blank=True, verbose_name="ID сессии Snowflake"
    )
    code = models.CharField(
        max_length=6, blank=True, null=True, verbose_name="Короткий код подтверждения"
    )
    purpose = models.CharField(
        max_length=20, choices=PURPOSE_CHOICES, verbose_name="Тип проверки"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["creation_id", "secure_token"]),
        ]
